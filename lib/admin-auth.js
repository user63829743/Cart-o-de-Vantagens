...
function isValidSession(request) {
  try {
    ...
    if (!secureEquals(signature, sign(payload, getAdminPassword()))) return false;
...
function setSessionCookie(response) {
  response.setHeader('Set-Cookie', `${SESSION_COOKIE}=${encodeURIComponent(createSessionToken())}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${SESSION_TTL_SECONDS}`);
}
...
module.exports = { clearSessionCookie, getAdminPassword, isValidSession, readBody, secureEquals, sendJson, setSessionCookie };
